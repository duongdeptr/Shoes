(function() {
  // VARS
  const productsContainer = document.querySelector("#t1");
  const cartContainer = document.querySelector("#shopping-cart");
  const cartContent = document.querySelector("#cart-content");
  const toggleCartBtn = document.querySelector("#icart");
  ...
  function saveProduct(clickedBtn) {
    ...
    const prodImage = card.querySelector(".pr-img img").src;
	  
    const prodName = cardInfo.querySelector(".pr-name").textContent;
    const prodPrice = cardInfo.querySelector(".new-price").textContent;
  ...
  productsContainer.addEventListener("click", function(e) {
    if (e.target.classList.contains("pr-btn")) {
      e.preventDefault();
